...
app.get('/api/catalogos/:id', authenticationToken, async (req, res) => {
	...
	if (id === '10venta') {
		sql = `SELECT "SucursalId","Sucursal" FROM sucursales WHERE "Status" = 'A' AND "TipoSucursal" IN ('S') ORDER BY "SucursalId"`
	}
...
app.post('/api/grabaventas', authenticationToken, async (req, res) => {
	...
	Este es el json que se recibe del frontend
	  ...
	  detalles: [
		{
		  SerialId: 1,
		  CodigoId: 63,
		  CodigoBarras: 'I000000000063',
		  Descripcion: 'LAVADA 30 MIN * * 0* 0PZA *',
		  Unidades: 1,
		  PrecioVentaConImpuesto: 50,
		  Accion: '?'
		}
	  ]
	}
	...
	*/
	...
	try {
		...
		await client.query('COMMIT')
		res.status(200).json({ FolioId: FolioId })
	} catch (error) {
		console.log(error.message)
		await client.query('ROLLBACK')
		res.status(500).json({ error: error.message })
